 ...
 * batch writes. This is the "follower path" — it writes raw bytes
 * directly to column families without any domain-object deserialization
 * or business logic re-execution.
 *
 * <p>Snapshot barriers are preserved: transitions with cmdType
 * {@code CreateSnapshot} or {@code SnapshotPurge} are applied in
 * isolated batches, matching the semantics of
 * {@link OzoneManagerDoubleBuffer#splitReadyBufferAtCreateSnapshot()}.
 */
public final class StateTransitionApplyEngine {
  ...
  /**
   * Applies a single {@link ReplicatedStateTransition} atomically.
   */
  ...
  /**
   * Applies a {@link BatchedStateTransitions} payload, splitting at
   * snapshot barriers to preserve checkpoint creation semantics.
   */
  ...
  /**
   * Splits transitions at snapshot barriers. CreateSnapshot and
   * SnapshotPurge must be applied in their own isolated batch to
   * ensure RocksDB checkpoint correctness.
   *
   * <p>Example: [t1, t2, snapCreate, t3, snapPurge, t4]
   * produces: [[t1, t2], [snapCreate], [t3], [snapPurge], [t4]]
   */
