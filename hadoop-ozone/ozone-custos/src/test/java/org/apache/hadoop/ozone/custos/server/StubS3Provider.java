...
/**
 * Test-only {@link CustosProvider} for {@link CredentialType#S3_SIGV4}. Echoes
 * the {@code accessId} attribute back as the authenticated identity.
 */
public class StubS3Provider implements CustosProvider {
  ...
  public CustosIdentity authenticate(CustosCredential credential)
    ...
    String accessId = credential.getAttribute("accessId");
    if (accessId == null) {
      throw new CustosException("stub-s3: missing accessId");
    }
    return CustosIdentity.newBuilder()
        .setSubject(accessId)
        .setAuthMethod("stub-s3")
        .build();
  }
}
